...
local function buildThreatSnapshot(root, humanoid)
	local threats = {}
	local byStep = table.create(PLAN_STEPS + EXTRA_ESCAPE_STEPS)
	for i = 1, PLAN_STEPS + EXTRA_ESCAPE_STEPS do
		byStep[i] = {}
	end
	...
	for _, data in pairs(ProjectileData) do
		...
		then
			...
			local maxIndexedStep =
				PLAN_STEPS + EXTRA_ESCAPE_STEPS

			for stepIndex = 1, maxIndexedStep do
...
Footer.Text = "2.0s MPC / Adaptive Beam / Time-index Threats"
