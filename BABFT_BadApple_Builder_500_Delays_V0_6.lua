...
local DISPLAY_START_COUNT = 4100
local DELAY_START_COUNT = 551
...
local BUILD_PAUSE = 0.04
local CONNECT_PAUSE = 0.04
local BATCH_PAUSE = 0.08
...
Title.Text = "BABFT Bad Apple Builder V0.6"
...
Info.Text = "64×64 / 3FPS / 250프레임\nDisplay 4100개 시작 · Delay 551개 시작 · 500개 수직 적층"
...
local function buildAll()
	...
	for y = 1, HEIGHT do
		...
		for x = 1, WIDTH do
			...
			setStatus(string.format(
				...
				index,
				totalDisplays,
				index / totalDisplays * 100
			)
			.. "\n보유량 인자: "
			.. tostring(DISPLAY_START_COUNT - (index - 1)))
			...
			local currentDisplayCount =
				DISPLAY_START_COUNT - (index - 1)

			if currentDisplayCount < 1 then
				return fail(
					"Display 보유량 계산 오류\n현재 값: "
					.. tostring(currentDisplayCount)
				)
			end
			...
			if not display then
				...
				display, errorMessage = installBlock(
					BuildRF,
					"DisplayBlock",
					currentDisplayCount,
					worldCFrame,
			...
			Generated.Displays[y][x] = display
	...
	for frameIndex = 1, frameCount do
		...
		local currentWhiteDelayCount =
			DELAY_START_COUNT - (whiteIndex - 1)

		if currentWhiteDelayCount < 1 then
			return fail(
				"Delay 보유량 계산 오류\n흰색 현재 값: "
				.. tostring(currentWhiteDelayCount)
			)
		end
		...
		local currentBlackDelayCount =
			DELAY_START_COUNT - (blackIndex - 1)

		if currentBlackDelayCount < 1 then
			return fail(
				"Delay 보유량 계산 오류\n검은색 현재 값: "
				.. tostring(currentBlackDelayCount)
			)
		end
