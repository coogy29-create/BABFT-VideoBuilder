...
local function waitForBindFire(display, timeout)
    local deadline = os.clock() + (timeout or 5)

    repeat
        if display and display.Parent then
            local bindFire = display:FindFirstChild("BindFire")
            if bindFire then
                return bindFire
            end
        end
...
local function bindDisplays(bindRF, delayBlock, displayBlocks)
    ...
    for index, display in ipairs(displayBlocks) do
        local bindFire = waitForBindFire(display, 5)

        if not bindFire then
            return false, "BindFire 준비 실패: Display " .. tostring(index), #targets
        end

        table.insert(targets, bindFire)
    end

    if #targets == 0 then
        return false, "BindFire 대상 없음", 0
    end
...
local function buildTest()
    ...
    local BuildingTool = Backpack:FindFirstChild("BuildingTool")
        or Character:FindFirstChild("BuildingTool")

    local PaintingTool = Backpack:FindFirstChild("PaintingTool")
        or Character:FindFirstChild("PaintingTool")

    local BindTool = Character:FindFirstChild("BindTool")

    if not BindTool then
        local backpackBindTool = Backpack:FindFirstChild("BindTool")

        if backpackBindTool then
            backpackBindTool.Parent = Character
            task.wait(0.2)
    ...
    local PropertiesTool = Character:FindFirstChild("PropertiesTool")
        or Backpack:FindFirstChild("PropertiesTool")
    ...
    if not BindTool then return fail("Character.BindTool을 찾지 못했습니다.") end
    ...
    print("흰색 연결 수:", wCount)
    print("검은색 연결 수:", bCount)
